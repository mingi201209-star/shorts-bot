...
def main():
    ...
    dedupe_marker = '''    if changed_keywords or changed_visual_goals or changed_missing_text or changed_endings:\n'''
    dedupe_replacement = '''    # If the model emits adjacent filler/repetition, repair only the later unlocked\n    # scene from its already-approved contract concepts. Never rewrite locked text.\n    for offset in range(1, len(scenes)):\n        current = scenes[offset]\n        previous = scenes[offset - 1]\n        contract = contracts[offset]\n        if not isinstance(current, dict) or not isinstance(previous, dict) or not isinstance(contract, dict):\n            continue\n        if contract.get("locked"):\n            continue\n        if _scene_text_similarity(previous.get("text"), current.get("text")) < 0.78:\n            continue\n        distinct = _deterministic_distinct_text(contract, offset + 1)\n        if distinct and _scene_text_similarity(previous.get("text"), distinct) < 0.78:\n            current["text"] = _formalize_common_ending(distinct)\n            changed_redundant_text += 1\n\n    if changed_keywords or changed_visual_goals or changed_missing_text or changed_redundant_text or changed_endings:\n'''
    text = replace_once(text, dedupe_marker, dedupe_replacement, "adjacent dedupe")

    log_marker = '''            f"keywords={changed_keywords} visual_goals={changed_visual_goals} "\n            f"missing_text={changed_missing_text} endings={changed_endings}"\n'''
    log_replacement = '''            f"keywords={changed_keywords} visual_goals={changed_visual_goals} "\n            f"missing_text={changed_missing_text} redundant_text={changed_redundant_text} "\n            f"endings={changed_endings}"\n'''
    text = replace_once(text, log_marker, log_replacement, "logging")

    RUNNER_PATH.write_text(text, encoding="utf-8")
    print("✅ Script Engine V2 nested aliases + repair envelope + visual_goal + missing-text + adjacent-dedupe recovery hotfix applied")
