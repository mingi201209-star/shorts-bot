...
Grounded facts are assigned to one scene each. Validation checks that every factual
scene realizes its owned claim without treating vocabulary shared by related claims
as evidence that the scene stole another claim.
"""
...
_CLAIM_TYPE_ORDER = {
    "mechanism_input": 10, "constraint": 15, "mechanism_change": 20,
    "mechanism_step": 30, "mechanism_effect": 35, "result": 40,
    "primary_result": 40, "tradeoff": 50, "payoff": 60,
}
...
def _effect_signatures(value: Any) -> set[str]:
    ...
    for index, raw in enumerate(raw_tokens):
        ...
        for previous in range(index - 1, max(-1, index - 4), -1):
            ...
            if len(candidate) < 2 or candidate in _STOP_TOKENS or _relation_for_token(candidate):
                continue
...
def _claim_matches_text(text: Any, claim: Dict[str, Any]) -> bool:
    ...
    if sentence_effects and claim_effects and sentence_effects & claim_effects:
        return True
    return max((_token_overlap(candidate, scope) for scope in _claim_scope_texts(claim)), default=0.0) >= 0.45


def _foreign_claim_matches(text: Any, foreign: Dict[str, Any], owned: Dict[str, Any], claims: Dict[str, Dict[str, Any]]) -> bool:
    """Require claim-discriminating evidence before declaring cross-scene migration.

    Run 33962777328 exposed that three related aircraft-window claims all shared
    subject/causal vocabulary. Legacy 0.45 overlap therefore marked every scene as
    every claim. A foreign match now needs either a foreign-specific effect relation
    or a token that distinguishes that claim from the owned claim and is not common
    to every grounded claim. This keeps real duplicate/outcome migration fail-closed.
    """
    candidate = _text(text)
    if not candidate:
        return False
    sentence_effects = _effect_signatures(candidate)
    foreign_effects = _claim_effect_signatures(foreign)
    owned_effects = _claim_effect_signatures(owned)
    if sentence_effects & (foreign_effects - owned_effects):
        return True

    sentence_tokens = set(_tokens(candidate))
    foreign_tokens = _claim_tokens(foreign)
    owned_tokens = _claim_tokens(owned)
    common_all = set.intersection(*(_claim_tokens(c) for c in claims.values())) if claims else set()
    discriminators = foreign_tokens - owned_tokens - common_all
    if not (sentence_tokens & discriminators):
        return False
    return max((_token_overlap(candidate, scope) for scope in _claim_scope_texts(foreign)), default=0.0) >= 0.45


def _valid_claim(raw: Any) -> bool:
    if not isinstance(raw, dict):
        return False
    if not _text(raw.get("claim_id")) or not _text(raw.get("claim_type")) or not _text(raw.get("evidence_summary")):
        return False
...
def extract_grounded_claims(candidate: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for position, raw in enumerate(trusted):
        ...
        claims.append({
            ...
            "allowed_paraphrase_scope": [_text(v) for v in raw.get("allowed_paraphrase_scope") or [] if _text(v)],
            "provenance_present": True,
            "_source_order": position,
        })
    claims.sort(key=lambda item: (_CLAIM_TYPE_ORDER.get(item["claim_type"], 35), item["_source_order"], item["claim_id"]))
    for item in claims:
...
def assign_claim_owners(claims: Iterable[Dict[str, Any]], *, first_scene: int = 3) -> List[Dict[str, Any]]:
    ...
    for offset, raw in enumerate(claims):
        ...
        used_ids.add(claim_id); used_scenes.add(owner)
...
def build_grounded_claim_plan(candidate: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    return assign_claim_owners(claims, first_scene=3) if claims else []
...
def validate_grounded_claim_usage(script: Dict[str, Any], plan: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    contracts = {int(item.get("index")): item for item in plan.get("contracts") or [] if isinstance(item, dict) and item.get("index") is not None}
    ...
    for scene_index, scene in enumerate(scenes, start=1):
        if isinstance(scene, dict):
            for signature in sorted(_effect_signatures(_text(scene.get("text")))):
    ...
    for signature, indexes in effect_occurrences.items():
        if len(indexes) >= 2:
            ...
            for offending in indexes[1:]:
                failures.append({"scene_index": offending, "reason": f"duplicate claim relation={signature} owner_scene={owner} offending_scene={offending}"})
    ...
    for scene_index, scene in enumerate(scenes, start=1):
        ...
        if not owned_id or owned_id not in claims:
            failures.append({"scene_index": scene_index, "reason": f"unplanned factual claim: scene {scene_index} has no grounded owned claim"})
            continue
        owned = claims[owned_id]
        if not bool(owned.get("provenance_present")):
            failures.append({"scene_index": scene_index, "reason": f"grounded claim {owned_id} is missing provenance"})
        if not _claim_matches_text(text, owned):
            failures.append({"scene_index": scene_index, "reason": f"unplanned factual claim: scene {scene_index} does not realize owned claim {owned_id}"})
        for other_id, other in claims.items():
            if other_id == owned_id:
                continue
            if _foreign_claim_matches(text, other, owned, claims):
                owner_scene = int(other.get("owner_scene") or 0)
                failures.append({"scene_index": scene_index, "reason": f"duplicate claim {other_id} owner_scene={owner_scene} offending_scene={scene_index}"})
        for signature in sorted(_effect_signatures(text) - planned_effects):
            failures.append({"scene_index": scene_index, "reason": f"unplanned factual claim: scene {scene_index} adds unsupported relation {signature}"})
    ...
    for failure in failures:
        ...
        if key not in seen:
            seen.add(key); deduped.append(failure)
    return deduped
