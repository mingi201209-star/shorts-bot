...
판단 기준은 "질문에 현상이 보이느냐"가 아니라
"질문만으로 Reveal의 핵심 메커니즘까지 사실상 답이 나와 있느냐"다.

중요:
- Core Question이 구체적인 관찰 현상/부품/조건을 이름으로 말하는 것은 정상이다.
  관찰 현상 자체를 질문에 적었다는 이유만으로 TOO_BROAD로 판정하지 마라.
- 질문에 없던 구체적인 구조, 하중/힘 전달 경로, 위치 차이, 조건, 예외,
  순서, 변형 모드, 또는 다른 비자명한 메커니즘을 Reveal이 새로 제공한다면
  그 후보는 NARROW_ENOUGH가 될 수 있다.
- 반대로 Reveal이 질문을 다시 말하거나
  "영향을 준다", "적응했다", "효율을 높인다", "안전성을 높인다",
  "메커니즘을 사용한다", "방법을 찾았다"처럼
  구체적인 인과 단계 없이 일반 목적/상식으로 끝나면 TOO_BROAD다.
- 질문 자체에 이미 Reveal의 핵심 원인/메커니즘이 거의 그대로 들어 있으면
  TOO_BROAD다.

예시:
TOO_BROAD
Question: "비행기 날개는 왜 공기 흐름을 최적화할까?"
Reveal: "공기 흐름을 최적화해 효율을 높이기 때문이다."

NARROW_ENOUGH
Question: "날개가 휘고 비틀릴 때 왜 단면의 유효 받음각까지 달라질 수 있을까?"
Reveal: "하중 아래의 bending/torsion 구조 변형이 날개 단면의 공탄성 각도를 바꾸기 때문이다."
