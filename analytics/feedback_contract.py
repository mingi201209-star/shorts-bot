...
def make_performance_snapshot(state="pending", *, collected_at=None, **metrics):
    ...
    unknown = set(metrics) - _SUPPORTED_METRICS - {"retention"}
    if unknown:
    ...
    for field in _SUPPORTED_METRICS:
        if field in metrics:
...
def make_video_lineage(
    ...
    *,
    ...
    """Create a lineage record without requiring analytics to exist yet."""
    if not lineage_id:
...
def normalize_video_lineage(record):
    ...
    for window in SNAPSHOT_WINDOWS:
        normalized["performance"].setdefault(
            window, make_performance_snapshot("pending")
        )
    return normalized
