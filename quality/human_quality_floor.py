...
Provider/model free: no budget, retry, model, or score-threshold changes.
"""
...
def opening_repeat_issue(scenes: List[Dict[str, Any]] | Any) -> str | None:
    ...
    if second_reasks_reason and len(shared) >= 2 and overlap >= 0.50:
        return "scene 2 verbosely re-asks scene 1 (shared=" + ",".join(sorted(shared)) + ")"
    return None
...
def is_aircraft_window_round_topic(candidate: Dict[str, Any] | Any) -> bool:
    ...
    values = [candidate.get("topic"), candidate.get("core_question"), micro.get("hook") if isinstance(micro, dict) else ""]
...
def normalize_aircraft_window_opening(candidate: Dict[str, Any]) -> Dict[str, Any]:
    ...
    issue = opening_repeat_issue([{"text": hook or "비행기 창문 모서리는 둥글게 되어 있습니다."}, {"text": question}])
    bland_hook = "둥글게 되어 있습니다" in hook or "모서리는 둥글" in hook or "모서리가 둥글" in hook
    ...
    payoff = str(micro.get("payoff", "")).strip().replace("동체 파열로 빠르게 이어질 수 있었습니다", "동체 파열로 진행될 수 있었습니다")
    if payoff:
        micro["payoff"] = payoff
