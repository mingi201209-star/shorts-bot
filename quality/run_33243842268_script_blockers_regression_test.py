"""Production counterexamples from Run 33243842268.

No model/network call is made. The test composes the same grounded Script hotfix
stack, then verifies semantic-state compaction and locked opening formalization.
"""
import importlib
import runpy
...
def _live_chevron_claims():
    return [
        _claim(
            "flow_interface",
            "mechanism_input",
            "Hot exhaust/core flow and cooler surrounding flow meet at the nozzle boundary.",
            ["The hot and cooler flows meet at the nozzle boundary."],
        ),
        _claim(
            "chevron_flow_mixing",
            "mechanism_change",
            "Chevron edges change how the two flows mix.",
            ["The chevrons change how the flows mix."],
        ),
        _claim(
            "mixing_transition",
            "mechanism_effect",
        ...
        _claim(
            "noise_reduction",
            "result",
            "The more gradual mixing reduces jet noise.",
            ["Chevron mixing reduces jet noise."],
        ),
    ]
...
def assert_a_live_chevrons_collapse_to_five_scenes():
    ...
    assert [item["claim_id"] for item in claims] == [
        "flow_interface", "chevron_flow_mixing", "noise_reduction"
    ], claims
    ...
    assert collapsed[0]["semantic_reason"] == (
        "same grounded semantic nucleus with modifier-only delta"
    ), collapsed

    change = next(
        item for item in claims if item["claim_id"] == "chevron_flow_mixing"
    )
    scopes = change["allowed_paraphrase_scope"]
    assert "The two flows mix more gradually across the boundary." in scopes, scopes

    result = next(item for item in claims if item["claim_id"] == "noise_reduction")
    assert result["owner_scene"] == 5, result
    assert sum(
        1 for item in claims if item["claim_id"] == "noise_reduction"
    ) == 1
...
def assert_b_distinct_downstream_state_stays_separate():
    claims = [
        ...
        _claim(
            "transition",
            "mechanism_effect",
            "A separated recirculation zone forms downstream.",
            ["A downstream recirculation zone forms."],
        ),
    ...
    assert [item["claim_id"] for item in plan["grounded_claim_plan"]] == [
        "input", "change", "transition", "result"
    ]
    assert plan["collapsed_grounded_claims"] == []
    transition = next(item for item in plan["contracts"] if item["index"] == 5)
    assert transition["causal_role"] == "mechanism_transition"
    assert transition["required_state_delta_evidence_backed"] is True
...
def assert_d_unsupported_drag_fuel_stability_still_fail():
    candidate = _candidate(_live_chevron_claims())
    ...
    assert any(term in reasons for term in ("항력", "연료", "안정")), reasons


def assert_e_locked_scene1_uses_shared_formal_corpus():
    candidate = _candidate(_live_chevron_claims())
    normalized = router._normalize_locked_candidate_narration(candidate)
    assert normalized["micro_narrative"]["hook"] == (
        "비행기 엔진 뒤쪽의 톱니 모양이 눈에 띕니다."
    ), normalized["micro_narrative"]
    assert normalized["micro_narrative"]["core_question"] == (
        "왜 비행기 엔진 뒤는 톱니처럼 생겼을까요?"
    )

    already_formal = _candidate(_live_chevron_claims())
    already_formal["micro_narrative"]["hook"] = "비행기 엔진 뒤쪽의 톱니 모양이 눈에 띕니다."
    preserved = router._normalize_locked_candidate_narration(already_formal)
    assert preserved["micro_narrative"]["hook"] == already_formal["micro_narrative"]["hook"]

    # Existing unsupported plain endings remain unsupported rather than being
    # generalized into a risky new grammar path; the speech validator may still
    # reject them exactly as before.
    assert formalize_declarative_text("플랩이 펼쳐진다.") == "플랩이 펼쳐진다."
    assert formalize_declarative_text("엔진이 돌아간다.") == "엔진이 돌아간다."
    assert formalize_declarative_text("비행기 엔진 뒤쪽 톱니") == "비행기 엔진 뒤쪽 톱니"
...
def main():
    ...
    print("RUN 33243842268 SCRIPT BLOCKERS REGRESSION: PASS")
