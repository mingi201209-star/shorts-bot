...
제목이나 첫 장면에는 시청자가 화면에서 보게 될
구체적인 대상(예: 창문, 스포일러, 톱니 모양 가장자리)이
이름으로 언급되는 것이 정상이다.
이것은 Visual 설계상 당연한 것이며 그 자체로는
낮은 점수의 근거가 아니다.

2번 기준 위반으로 판단해야 하는 경우는
"그 대상이 왜 그런 모습/역할을 하는지"에 대한
실제 이유(mechanism, reveal)까지
제목만으로 예상할 수 있을 때뿐이다.
대상의 이름이 제목에 있다는 이유만으로
2번 기준을 위반했다고 판단하지 마라.

점수는 다음을 참고해 판단한다.

0~2: 거의 모든 시청자가 결과와 이유를
     이미 정확히 알고 있다.
...
7~8: 결과와 이유 모두 대부분의 시청자에게 낯설고,
     알고 나면 공유하고 싶어진다.

9~10: 이 사실을 정확히 아는 사람이
      거의 없을 정도로 드문 정보다.
"""
