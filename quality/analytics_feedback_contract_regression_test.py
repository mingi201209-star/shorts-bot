from analytics.feedback_contract import make_performance_snapshot, make_video_lineage, normalize_video_lineage


def test_zero_is_not_unknown():
    snapshot = make_performance_snapshot("complete", views=0, engaged_views=0, subscriber_gain=0)
...
def test_24h_and_72h_are_independent():
    record = make_video_lineage(
        ...
        snapshots={"24h": {"state": "complete", "views": 1200, "engaged_views": 700, "subscriber_gain": 3}},
    )
    assert record["performance"]["24h"]["state"] == "complete"
    assert record["performance"]["72h"]["state"] == "pending"


def test_collection_states_are_explicit():
    assert make_performance_snapshot("partial", views=100)["shares"] is None
    assert make_performance_snapshot("unavailable")["views"] is None


def test_lineage_keeps_metadata():
    record = make_video_lineage(
        "candidate-7:run-99",
...
def test_legacy_record_is_backward_compatible():
    normalized = normalize_video_lineage({"lineage_id": "old-1", "production": {"run_id": 1}})
...
def main():
    ...
    test_collection_states_are_explicit()
    test_lineage_keeps_metadata()
