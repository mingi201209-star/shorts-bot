...
def main():
    ...
    try:
        ...
        def fake_v2(item):
            ...
            return {"engine": "v2", "title": "윙렛의 이유", "scenes": scenes}
        ...
        question_result = router.generate_script({"topic": "aviation", "category": "항공"}, question_item)
        ...
        statement_topic_item["micro_narrative"]["hook"] = "왜 비행기 날개는 일부러 휘어지게 만들까요?"
        original_statement_question = statement_topic_item["micro_narrative"]["hook"]
        statement_result = router.generate_script({"topic": "aviation", "category": "항공"}, statement_topic_item)
        assert seen_candidates[-1]["micro_narrative"]["hook"] == "비행기 날개는 일부러 휘어지게 만듭니다."
        assert statement_result["scenes"][0]["text"] == "비행기 날개는 일부러 휘어지게 만듭니다."
        assert statement_topic_item["micro_narrative"]["hook"] == original_statement_question

        danda_item = candidate()
        danda_item["topic"] = "비행기 엔진은 날개 아래에 단다"
        danda_item["micro_narrative"]["hook"] = "왜 비행기 엔진은 날개 아래에 장착될까요?"
        original_danda_question = danda_item["micro_narrative"]["hook"]
        danda_result = router.generate_script({"topic": "aviation", "category": "항공"}, danda_item)
        assert seen_candidates[-1]["micro_narrative"]["hook"] == "비행기 엔진은 날개 아래에 답니다."
        assert danda_result["scenes"][0]["text"] == "비행기 엔진은 날개 아래에 답니다."
        ...
        original_locked = {"core_question": locked_item["core_question"], "micro_narrative": dict(locked_item["micro_narrative"])}
        locked_result = router.generate_script({"topic": "aviation", "category": "항공"}, locked_item)
        ...
        # Production Run 33185606044: locked reveal ended in plain-form `줄인다`.
        # Runner intentionally cannot mutate locked narration, so the router must
        # formalize this exact common ending before V2 freezes the contract.
        run_33185606044 = candidate()
        run_33185606044["micro_narrative"] = {
            "hook": "비행기 날개 끝이 위로 꺾여 있습니다.",
            "reveal": "날개 끝에서 압력 차가 강한 소용돌이를 만들어내고, 위로 꺾인 형태가 그 소용돌이를 줄인다.",
            "payoff": "결과적으로 비행기의 연료 효율이 개선됩니다.",
        }
        original_reveal = run_33185606044["micro_narrative"]["reveal"]
        router.generate_script({"topic": "aviation", "category": "항공"}, run_33185606044)
        production_normalized = seen_candidates[-1]
        assert production_normalized["micro_narrative"]["reveal"] == (
            "날개 끝에서 압력 차가 강한 소용돌이를 만들어내고, 위로 꺾인 형태가 그 소용돌이를 줄입니다."
        )
        assert run_33185606044["micro_narrative"]["reveal"] == original_reveal
