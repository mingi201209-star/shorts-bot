...
# No hard scene quota in the Writer contract.
...
# Every intermediate scene must advance information or visual meaning.
for phrase in (
    "새 사실",
    "새 원인 또는 메커니즘",
    "새 의문",
...
# Generic summary wording is not a hard-ban; only filler use is forbidden.
...
# Duration is preference-only and must stop when the explanation is complete.
...
# The mutated production prompt must no longer contain the legacy fill-duration command.
...
legacy_length = "전체 TTS가 {TARGET_MIN_SECONDS}~{TARGET_MAX_SECONDS}초가 되도록 충분한 문장 분량을 만든다."
assert legacy_length not in source

# Existing V1 compressor contract remains available; V2 adds no deletion taxonomy.
